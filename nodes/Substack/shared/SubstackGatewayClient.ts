...
interface SubstackGatewayConfig {
...
class GatewayHttpClient {
	...
	constructor(config: SubstackGatewayConfig) {
...
class GatewayOwnProfileContext extends GatewayProfileContext {
	...
	async *following(): AsyncIterable<any> {
	...
	async publishNote(content: string, options?: { attachment?: string }): Promise<{ id: number }> {
...
class GatewayPostContext {
	...
	async *comments(): AsyncIterable<any> {
...
export class SubstackClient {
	...
	async profileForSlug(slug: string): Promise<any> {
		if (!slug || slug.trim() === '') {
